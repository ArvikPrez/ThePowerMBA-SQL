...
-- Se ha corregido el uso del igual por una busqueda con patrón (like/ilike) para que en este caso el apellido contenga la cadena Allen y no sea una coincidencia exacta.
...
offset 2
limit 1;
-- Se ha corregido el valor del offset, puesto que el valor 0 es el último, el valor 1 es el penultimo, y el valor 2 el antepenultimo. 
...
select f.title , f.length, c.category_id 
from film f 
...
and f.length >180;
-- Efectivamente, en este caso me faltaba añadir la duracion de las peliculas, se ha añadido la condición de que la duracion sea mayor a 180.
...
max(a.actor_id)
from actor a; 
-- Cambiado el orden de la consulta tal cual se pide en el enunciado. Primero el minimo y luego el maximo. 
...
-- Corregido el orden, efectivamente me equivoqué y lo puse descendente en lugar de ascendente.
